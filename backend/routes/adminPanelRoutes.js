...
const activityMappings = {
  'Scholars': {
    table: 'scholars',
    alias: 's',
    columns: ['id', 'scholar_name', 'scholar_type', 'institute', 'university', 'title', 'domain', 'phd_registered_year', 'completed_year', 'status', 'publications', 'created_at'],
    joinQuery: `
  ...
  'Consultancy': {
    ...
    alias: 'cp',
    columns: ['id', 'pi_name', 'co_pi_names', 'project_title', 'industry', 'from_date', 'to_date', 'amount', 'organization_name', 'created_at'],
    joinQuery: `
  ...
  'Funded Project': {
    ...
    alias: 'pp',
    columns: ['id', 'pi_name', 'co_pi_names', 'project_title', 'funding_agency', 'from_date', 'to_date', 'amount', 'organization_name', 'created_at'],
    joinQuery: `
  ...
  'Events Attended': {
    ...
    alias: 'ea',
    columns: ['id', 'programme_name', 'title', 'from_date', 'to_date', 'mode', 'organized_by', 'participants', 'financial_support', 'support_amount', 'created_at'],
    joinQuery: `
  ...
  'Industry Knowhow': {
    ...
    alias: 'ik',
    columns: ['id', 'internship_name', 'title', 'company', 'outcomes', 'from_date', 'to_date', 'venue', 'participants', 'financial_support', 'support_amount', 'created_at'],
    joinQuery: `
  ...
  'Certification Courses': {
    ...
    alias: 'cc',
    columns: ['id', 'course_name', 'forum', 'from_date', 'to_date', 'days', 'certification_date', 'created_at'],
    joinQuery: `
  ...
  'Publications': {
    table: 'book_chapters',
    alias: 'bc',
    columns: ['id', 'publication_type', 'publication_name', 'publication_title', 'authors', 'index_type', 'doi', 'citations', 'publisher', 'page_no', 'publication_date', 'impact_factor', 'created_at'],
    joinQuery: `
  ...
  'Events Organized': {
    ...
    alias: 'eo',
    columns: ['id', 'program_name', 'program_title', 'coordinator_name', 'co_coordinator_names', 'speaker_details', 'from_date', 'to_date', 'days', 'sponsored_by', 'amount_sanctioned', 'participants', 'created_at'],
    joinQuery: `
  ...
  'Patent/Product Development': {
    ...
    alias: 'pat',
    columns: ['id', 'project_title', 'patent_status', 'month_year', 'working_model', 'prototype_developed', 'created_at'],
    joinQuery: `
  ...
  'Education': {
    ...
    columns: [
      ...
      'phd_university', 'phd_title', 'phd_guide_name', 'phd_college', 'phd_status', 'phd_registration_year', 'phd_completion_year',
  ...
  'Personal Information': {
    table: 'personal_information', // Ensure this matches the database table name
    alias: 'pi',
    columns: [
      'id', 'Userid', 'full_name', 'date_of_birth', 'age', 'gender', 'email', 
      'mobile_number', 'communication_address', 'permanent_address', 'religion', 
      'community', 'caste', 'post', 'department', 'created_at', 'updated_at'
    ],
    joinQuery: `
...
router.get('/admin-panel/staff-with-activities', async (req, res) => {
  try {
    ...
    const [staffMembers] = await sequelize.query(`
      SELECT u.Userid, u.username, u.email, u.staffId, u.Deptid, u.image
      FROM users u 
      WHERE u.role = 'Staff' 
      ORDER BY u.username
    `);

    console.log(`Found ${staffMembers ? staffMembers.length : 0} staff members`);
    ...
    if (viewMode === 'staff') {
      ...
      data.forEach(staff => {
        worksheet.addRow({
          ...
          activities: staff.activities ? staff.activities.join(', ') : 'None'
        });
      });
