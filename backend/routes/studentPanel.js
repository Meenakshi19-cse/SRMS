...
router.post('/student-admin-panel/export-excel', async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=student_activities_export.xlsx');
  ...
  try {
    ...
    for (const [activityName, mapping] of Object.entries(studentActivityMappings)) {
      try {
        ...
        stats[activityName] = result[0].count;
      } catch (error) {
    ...
    stats.totalStudents = studentCount[0].count;
