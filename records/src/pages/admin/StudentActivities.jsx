...
import { FaSearch, FaUserGraduate, FaUndo, FaEye, FaFileExport, FaUsers, FaCalendarAlt, FaCertificate, FaBook, FaAward, FaSchool, FaMedal, FaFilter, FaChevronDown } from "react-icons/fa";
...
const fetchStudentData = async (departmentId = null) => {
  ...
  const getActivityIcon = (activity) => {
    const iconMap = {
      ...
      'Online Courses': <FaCertificate className="w-4 h-4" />,
      'Achievements': <FaMedal className="w-4 h-4" />,
      'Internships': <FaSchool className="w-4 h-4" />,
      'Scholarships': <FaBook className="w-4 h-4" />,
      'Student Details': <FaUserGraduate className="w-4 h-4" />
    };
    return iconMap[activity] || <FaUsers className="w-4 h-4" />;
  };
