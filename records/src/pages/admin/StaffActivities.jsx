...
import { FaSearch, FaUserTie, FaUndo, FaEye, FaFileExport, FaUsers, FaFileUpload, FaCalendarAlt, FaCertificate, FaBook, FaAward, FaUserGraduate, FaChevronDown, FaFilter } from "react-icons/fa";
...
function AdminPanel() {
  ...
  const [selectedActivityField, setSelectedActivityField] = useState(["ID","Staff Name","Department"]);
  ...
  // Fetch available fields for selected activity
    ...
    const fetchActivityFields = async (activityName) => {
      try {
        ...
        // Fetch fields from the new API endpoint
        ...
        console.log(`Fetched fields for ${activity.table}`);
        const fieldsData = await response.json();
        const cols = Array.isArray(fieldsData?.columns) ? fieldsData.columns : [];
        setActivityFields(cols);  
        
        // Define default fields that should always be displayed
        const defaultFields = ['S.NO', 's_no', 'staff_name', 'department'];
        
        // Combine default fields with fetched fields
        const selectableFields = [...defaultFields, ...cols.filter(field => 
        ...
        setSelectedActivityFields(defaultFields); // Set default fields as selected
      } catch (error) {
  ...
  // Load the related fields
  const fields = Object.keys(activity); // Or however you get availableActivityFields
  setAvailableActivityFields(fields);

  // Store the whole activity data (or fetch it from API)
  setActivityData([activity]); // Wrap in array so we can map in table
};
  ...
  const handleActivityFieldToggle = (fieldName) => {
    // Define default fields that cannot be deselected
    const defaultFields = ['S.NO', 's_no', 'staff_name', 'department'];
    
    // Prevent deselection of default fields
    if (defaultFields.some(defaultField => fieldName.toLowerCase() === defaultField.toLowerCase())) {
      return; // Do nothing if trying to deselect a default field
    }
    ...
    setSearchStaffName(""); // Reset staff name search
    fetchStaffData(departmentId); // Fetch staff based on selected department
  };

  // Select/Deselect all activity fields
  const handleSelectAllActivityFields = () => {
    // Define default fields that should always be selected
    const defaultFields = ['S.NO', 's_no', 'staff_name', 'department'];
    
    if (selectedActivityFields.length === availableActivityFields.length) {
      // When deselecting all, keep only the default fields selected
      setSelectedActivityFields(defaultFields);
    } else {
      // When selecting all, include all available fields
  ...
  const fetchStaffData = async (departmentId) => {
    try {
      const response = await fetch(`${backendUrl}/api/admin-panel/staff-with-activities`);
  ...
  const fetchActivityData = async (activityName, departmentId = null, staffName = null, fields = null) => {
    try {
      ...
      console.log('Activity data response:', responseData);
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      try {
        ...
        // Set activities from the predefined activityList
  ...
  const handleSearch = async () => {
    try {
      if (searchActivity) {
        // If activity is selected, fetch activity-specific data
        const selectedDept = departments.find(dept => 
          dept.Deptacronym.toLowerCase() === searchDepartment.toLowerCase()
        );
        await fetchActivityData(searchActivity, selectedDept?.Deptid, searchStaffName, selectedActivityFields);
      } else {
        // If no activity selected, show staff data with filters
  ...
  const renderActivityTable = () => {
    ...
    const existingDefaultColumns = activityColumns.filter(column => 
      defaultFields.some(defaultField => 
        column.toLowerCase() === defaultField.toLowerCase()
      )
    );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                {staffData
                  .filter(staff => !searchDepartment || staff.Deptid === departments.find(dept => dept.Deptacronym === searchDepartment)?.Deptid)
                  .map((staff) => (
            ...
            {/* Activity Fields Filter (only shown when activity is selected) */}
