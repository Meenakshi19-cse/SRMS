...
const FormSection = ({ formData, handleInputChange, isEditMode }) => (
  ...
  <Field
    ...
    value={formData.referral_person_name}
    onChange={handleInputChange}
    placeholder="Enter referral name"
  />
  <Field
    label="Referral Person Role"
    name="referral_person_role"
    value={formData.referral_person_role}
    onChange={handleInputChange}
    placeholder="Enter referral role"
  />
  <Field
    label="Referral Person Mobile"
    name="referral_person_mobile"
    value={formData.referral_person_mobile}
    onChange={handleInputChange}
    type="tel"
...
const StudentInternship = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.status === "completed" && !formData.certificate_file) return toast.error("Please upload a certificate before marking as 'Completed'.");
    ...
    formToSubmit.append("referral_person_name", formData.referral_person_name || "");
    formToSubmit.append("referral_person_role", formData.referral_person_role || "");
    formToSubmit.append("referral_person_mobile", formData.referral_person_mobile || "");

    // Append certificate file if it exists
    if (formData.certificate_file) formToSubmit.append("certificate_file", formData.certificate_file);
    console.log(formData)
    try {
      if (isEditMode) {
        if (formData.status === "completed" && !formData.certificate_file) return toast.error("Certificate is required when updating to 'Completed'.");
        ...
        <table className="min-w-full bg-white border border-gray-200 rounded-lg shadow-sm">
          <thead>
            <tr className="bg-[#4f46e5] text-white">
              ...
              <th className="py-3 px-4 text-left font-medium">Referral</th>
              ...
              >
                ...
                <td className="py-3 px-4 text-gray-700">
                  {internship.referral_person_name
                    ? `${internship.referral_person_name} (${internship.referral_person_role || "N/A"}, ${internship.referral_person_mobile || "N/A"})`
                    : "N/A"}
                </td>
